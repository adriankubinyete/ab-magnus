...
module.exports = (mqList, absqFind, mqBlock, mqUnblock, mqDisable, mqEnable, dqNotify) => {

    mqList.process(MQLIST_CONC, async(job, done) => {
        const qlog = generateLogger(`LIST:${job.id}`, path.resolve(__dirname, MAG_LOG_LOCATION), MAG_LOG_LEVEL, MAG_LOG_FILE_LEVEL, MAG_LOG_FILE_ROTATE);
        ...
        if(job.progress()>=100) {
            ...
            absqFind.add({hasContract, doesntHaveContract})
    
        }
    
    });

    mqBlock.process(1, async(job, done) => {
        const qlog = generateLogger(`BLOCK:${job.id}`, path.resolve(MAG_LOG_LOCATION), MAG_LOG_LEVEL, MAG_LOG_FILE_LEVEL, MAG_LOG_FILE_ROTATE);
    ...
    mqUnblock.process(1, async(job, done) => {
        const qlog = generateLogger(`UNBLOCK:${job.id}`, path.resolve(MAG_LOG_LOCATION), MAG_LOG_LEVEL, MAG_LOG_FILE_LEVEL, MAG_LOG_FILE_ROTATE);
        qlog.debug(`Starting job "${job.id}"`);
    ...
    mqDisable.process(1, async(job, done) => {
        const qlog = generateLogger(`DISABLE:${job.id}`, path.resolve(MAG_LOG_LOCATION), MAG_LOG_LEVEL, MAG_LOG_FILE_LEVEL, MAG_LOG_FILE_ROTATE);
        qlog.debug(`Starting job "${job.id}"`);
    ...
    mqEnable.process(1, async(job, done) => {
        const qlog = generateLogger(`ENABLE:${job.id}`, path.resolve(MAG_LOG_LOCATION), MAG_LOG_LEVEL, MAG_LOG_FILE_LEVEL, MAG_LOG_FILE_ROTATE);
        qlog.debug(`Starting job "${job.id}"`);
    ...
    log.info("Processor for magnus.js registered successfully");
}
